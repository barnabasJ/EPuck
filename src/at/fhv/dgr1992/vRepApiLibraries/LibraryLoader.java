...
public class LibraryLoader {
    ...
    public static void loadLibrary(){
        if(OSValidator.isWindows()){
            ...
            loadLib("Mac"+File.separator+"lib"+LIBREMOTEAPIJAVA+".dylib","dylib");
        } else if(OSValidator.isUnix() || OSValidator.isSolaris()){
            if(OSValidator.isLinux64Bit()) {
                loadLib("Linux"+File.separator+"64Bit"+File.separator+"lib"+LIBREMOTEAPIJAVA+".so","so");
            } else {
                loadLib("Linux"+File.separator+"32Bit"+File.separator+"lib"+LIBREMOTEAPIJAVA+".so","so");
